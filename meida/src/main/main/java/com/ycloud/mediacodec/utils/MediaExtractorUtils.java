...
public class MediaExtractorUtils {
    ...
    public static int getFirstVideoTrack(MediaExtractor extractor) {
        ...
        return trackResult.mVideoTrackIndex;
    }
    ...
    public static int getFirstAudioTrack(MediaExtractor extractor) {
        ...
        for (int i = 0; i < trackCount; i++) {
            MediaFormat format = extractor.getTrackFormat(i);
            String mime = format.getString(MediaFormat.KEY_MIME);
            if (trackResult.mAudioTrackIndex < 0 && mime.startsWith("audio/")) {
                trackResult.mAudioTrackIndex = i;
                trackResult.mAudioTrackMime = mime;
                trackResult.mAudioTrackFormat = format;
            }
            if ( trackResult.mAudioTrackIndex >= 0) break;
        }
        if (trackResult.mAudioTrackIndex < 0) {
            throw new IllegalArgumentException("extractor does not contain audio tracks.");
        }
        return trackResult.mAudioTrackIndex;
    }
}
