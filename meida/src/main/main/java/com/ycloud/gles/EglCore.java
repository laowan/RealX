...
public final class EglCore implements IEglCore
{
     ...
     */
    protected EglCore(/*EGLContext*/ Object sharedContext, int flags) {
        ...
        if ((flags & FLAG_TRY_GLES3) != 0) {
            ...
            if (config != null) {
                ...
                EGLContext context = EGL14.eglCreateContext(mEGLDisplay, config, shareEGLContext,
                        attrib3_list, 0);
    ...
    private EGLConfig getConfig(int flags, int version) {
        ...
        if (!EGL14.eglChooseConfig(mEGLDisplay, attribList, 0, configs, 0, configs.length,
                numConfigs, 0)) {
            YYLog.error(TAG, "unable to find RGB8888 / " + version + " EGLConfig");
            return null;
        }
        return configs[0];
    }
    ...
    public void setPresentationTime(IEglSurfaceBase eglSfBase, long nsecs) {
        ...
        if(eglSfBase != null && (eglSfBase instanceof  EglSurfaceBase)) {
            ...
            _setPresentationTime(eglSurface, nsecs);
    ...
    protected void releaseSurface(EGLSurface eglSurface) {
        boolean ret = true;
        if (mEGLDisplay == EGL14.EGL_NO_DISPLAY) {
            Log.d(TAG, "NOTE: releaseSurface w/o display");
            return;
        }

        try {
            ret = EGL14.eglDestroySurface(mEGLDisplay, eglSurface);
            YYLog.info(this, "[GlUtil] releaseSurface success? " + ret);
        } catch (Exception e) {
            YYLog.error(TAG, "eglDestroySurface exception:" + e.getMessage());
        }
    }
    ...
    public int _querySurface(EGLSurface eglSurface, int what) {
        int[] value = new int[1];
        boolean result = EGL14.eglQuerySurface(mEGLDisplay, eglSurface, what, value, 0);

        YYLog.info(this, "[Encoder] [double-check] query surface result:"+result);
        return value[0];
    }
