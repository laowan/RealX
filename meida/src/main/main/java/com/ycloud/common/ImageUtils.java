...
public class ImageUtils {
	...
	public static class Font {
		...
		public Font(String content, int size, int color, int x, int y, boolean isBold, Typeface typeface) {
			this.content = content;
			this.size = size;
			this.color = color;
			this.x = x;
		...
		public Font(int index, String content, int size, int color, int x, int y, boolean isBold, Typeface typeface) {
			this.index = index;
			this.content = content;
			this.size = size;
			this.color = color;
	...
	public void textToImg(TextImg textImg, ArrayList<Font> fontList) {
		...
		try {
			...
			for (Font font : fontList) {
				...
				/*
				 * int baseY = (int) ((canvas.getHeight() / 2) - ((paint.descent() + paint.ascent()) / 2)); Log.i("hyq", "--color:" + font.color + " size:" + font.size + " x:" + font.x + " y:" +
				 * font.y + " baseline:" + baseline + " baseY:" + baseY + " content:" + font.content + " index:" + font.index + " textImg.width:"+ textImg.width + " textImg.height:" + textImg.height);
				 * Log.i("hyq", "paint baselineShift:" + paint.baselineShift + " ascent:" + paint.ascent() + " descent:" + paint.descent()); Log.i("hyq", "fontMetrics top:" + fontMetrics.top +
				 * " ascent:" + fontMetrics.ascent + " descent:" + fontMetrics.descent + " bottom:" + fontMetrics.bottom+ " leading:" + fontMetrics.leading);
				 */
			}
			canvas.save(Canvas.ALL_SAVE_FLAG);
			canvas.restore();
