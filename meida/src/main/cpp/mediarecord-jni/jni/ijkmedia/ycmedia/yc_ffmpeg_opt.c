...
//add bhl end
#include "common.h"
#include "libavcodec/utils_jni.h"
...
{
	av_log(NULL, AV_LOG_DEBUG, "opt=%s arg=%s", opt, arg);
    OptionsContext *o = optctx;
...
static int open_input_file(OptionsContext *o, const char *filename)
{
	...
	int j;
    for (j = 0; j < o->nb_codec_names; j++) {
        char *spec = o->codec_names[j].specifier;
        av_log(NULL, AV_LOG_DEBUG, "open_input_file spec=%s", spec);
    }
...
{
	av_log(NULL, AV_LOG_DEBUG, "enter into %s. filename=%s nb_codec_names=%d", __FUNCTION__, filename, o->nb_codec_names);
    {
        int j;
        for (j = 0; j < o->nb_codec_names; j++) {
            char *spec = o->codec_names[j].specifier;
            av_log(NULL, AV_LOG_INFO, "open_output_file spec=%s", spec);
        }
    }
...
static int opt_video_decoder_pixfmt(void *optctx, const char *opt, const char *arg)
{
    av_log(NULL, AV_LOG_WARNING, "frame_pix_fmt opt=%s arg=%s\n", opt, arg);
    return 0;
}
