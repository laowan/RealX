...
extern log_callback_ffmpeg(void * ptr, int level, const char * fmt, va_list vargs);
...
{
    ALOGI("Simple multimedia streams analyzer");
	ALOGI("usage: %s [OPTIONS] [INPUT_FILE]", program_name);
}
...
char* ffprobe_main(int argc, char **argv)
{
	//bhl
	if(ffmpeg_running) {
        av_log(NULL, AV_LOG_ERROR, "ffprobe error : ffmpeg is running");
		return NULL;
	}
	ffmpeg_running = 1;
	//bhl end
    ...
    if (ret < 0) {
        free(xprintf_buf);
        xprintf_buf = NULL;
        av_log(NULL, AV_LOG_ERROR, "probe error, set ffmpeg_running 0, return\n");
		ffmpeg_running = 0;//bhl
    	return NULL;
    } else {
		if (strlen(xprintf_buf) < 1024) {
            av_log(NULL, AV_LOG_DEBUG, "buf_size:%d, content:\n %s", strlen(xprintf_buf), xprintf_buf);
		} else {
			...
			while(i+1000<len) {
                ...
                av_log(NULL, AV_LOG_DEBUG, "%s", xprintf_buf+i);
		...
		ffmpeg_running = 0;//bhl
		av_log(NULL, AV_LOG_DEBUG, "ffprobe end:set ffmpeg_running 0.\n");
		return xprintf_buf;
    }
}
