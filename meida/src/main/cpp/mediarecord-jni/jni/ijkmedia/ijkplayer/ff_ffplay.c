...
static void video_display2(FFPlayer *ffp)
{
    ...
    if (is->video_st) {
        ...
        av_log(ffp, AV_LOG_DEBUG, "stream_display2 but video_st is empty");
    }
}
...
static void sync_clock_to_slave(Clock *c, Clock *slave)
{
    ...
    if (!isnan(slave_clock) && (isnan(clock) || fabs(clock - slave_clock) > AV_SYNC_THRESHOLD_MAX))//bhl
...
{
    av_log(NULL, AV_LOG_DEBUG, "stream_toggle_pause_l start pause_on = %d\n", pause_on);
    VideoState *is = ffp->is;
...
{
    VideoState *is = ffp->is;
    av_log(NULL, AV_LOG_DEBUG, "stream_update_pause_l start step = %d, pause_req = %d, buffering_on = %d\n", is->step, is->pause_req, is->buffering_on);
...
display:
            ...
            if (is->step) {
                is->step = 0;
                if (!is->paused) {
                    av_log(NULL, AV_LOG_DEBUG, "video_refresh step\n");
                    stream_update_pause_l(ffp);
                }
            }
            SDL_UnlockMutex(ffp->is->play_mutex);
...
static int configure_video_filters(AVFilterGraph *graph, FFPlayer *ffp, const char *vfilters, AVFrame *frame)
{
    ...
    int ret;
    VideoState *is = ffp->is;
    AVFilterContext *filt_src = NULL, *filt_out = NULL, *last_filter = NULL, *head_filter = NULL;
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            if(graph == NULL) {
                av_log(NULL, AV_LOG_ERROR,"graph mem error");
            }

            if ((ret = configure_video_filters(graph, ffp, ffp->vfilters, frame)) < 0) {
                int i=0;
                for(;i<3;i++){
                    configure_video_filters(graph, ffp, NULL, frame);
                    if ((ret = configure_video_filters(graph, ffp, ffp->vfilters, frame)) >= 0) {
                        break;
                    }
                }

                av_log(NULL, AV_LOG_ERROR,"configure_video_filters error,loopcound:%d",i);
                if(ret<0){
                    //ffp_notify_msg2(ffp, FFP_MSG_ERROR, ffp->last_error);
                    //goto the_end;
                    ffp->refresh_vfilters = 1;
                     continue;
                }
            }
...
//      of the seek_pos/seek_rel variables
            ...
            SDL_LockMutex(ffp->is->play_mutex);
            if (ffp->auto_resume) {
                av_log(NULL, AV_LOG_DEBUG, "read_thread auto_resume = %d\n", ffp->auto_resume);
                is->pause_req = 0;
                is->buffering_on = 1;
                ffp->auto_resume = 0;
                stream_update_pause_l(ffp);
...
{
    av_log(NULL, AV_LOG_DEBUG, "stream_open start\n");
    assert(!ffp->is);
    VideoState *is;
...
{
    av_log(NULL, AV_LOG_DEBUG, "ffplay_video_refresh_thread start\n");
    FFPlayer *ffp = arg;
    VideoState *is = ffp->is;
...
#if CONFIG_AVFILTER
int  ffp_set_vfilters(FFPlayer *ffp, const char *vfilters) {
    if (ffp) {
        char *tmp = NULL;
        if (vfilters) {
            int len = strlen(vfilters);
            tmp = malloc(len+1);
            if (tmp == NULL) {
                av_log(ffp, AV_LOG_ERROR, "vfilters mem out");
                return -1;
             }
            strcpy(tmp, vfilters);
        }
...
int ffp_start_from_l(FFPlayer *ffp, long msec)
{
    ...
    av_log(ffp, AV_LOG_DEBUG, "stream_start_from end");
...
int ffp_start_l(FFPlayer *ffp)
{
    ...
    av_log(ffp, AV_LOG_DEBUG, "stream_start end");
...
int ffp_pause_l(FFPlayer *ffp)
{
    ...
    VideoState *is = ffp->is;
    if (!is) {
        av_log(ffp, AV_LOG_DEBUG, "stream_pause but VideoState is null");
        return EIJK_NULL_IS_PTR;
    }

    av_log(ffp, AV_LOG_DEBUG, "stream_pause and display2");
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    VideoState *is = ffp->is;
    if (!is) {
        av_log(ffp, AV_LOG_DEBUG, "stream_seek but VideoState is null");
        return EIJK_NULL_IS_PTR;
    }
    ...
    // FIXME: 9 seekable
    av_log(ffp, AV_LOG_DEBUG, "stream_seek and display2 %"PRId64"(%d) + %"PRId64", \n", seek_pos, (int)msec, start_time);
    stream_seek(is, seek_pos, 0, 0);

    // seek to and force refresh
    video_display2(ffp);
