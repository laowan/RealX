...
#define JNI_MODULE_PACKAGE      "com/ycloud/player"
#define JNI_CLASS_IJKPLAYER     "com/ycloud/player/IjkMediaPlayer"
#define JNI_IJK_MEDIA_EXCEPTION "com/ycloud/player/IjkMediaException"
...
typedef struct IJKFormatSegmentConcatContext {    
	/**     * Segment position (0-based) in stream     *     * set by demux     */    
	int count;
...
static int
IjkMediaPlayer_setVFilters(JNIEnv *env, jobject thiz, jstring vfilters) {
      MPTRACE("%s\n", __func__);
      IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
      const char* c_vfilters = NULL;
      int ret = -1;
      JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: setVFilters: null mp", LABEL_RETURN);

      if (vfilters != NULL) {
        c_vfilters = (*env)->GetStringUTFChars(env, vfilters, NULL );
        JNI_CHECK_GOTO(c_vfilters, env, "java/lang/OutOfMemoryError", "mpjni: setVFilters: vfilters.string oom", LABEL_RETURN);
      }
      
      ret = ijkmp_set_vfilters(mp, c_vfilters);
    
      LABEL_RETURN:
      if (c_vfilters)
          (*env)->ReleaseStringUTFChars(env, vfilters, c_vfilters);
      ijkmp_dec_ref_p(&mp);
      return ret;
}
...
static JNINativeMethod g_methods[] = {
    ...
    { "_setVFilters", "(Ljava/lang/String;)I", (void *) IjkMediaPlayer_setVFilters },
};
