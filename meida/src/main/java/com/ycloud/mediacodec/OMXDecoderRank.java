...
//        Log.i(OMXConstant.TAG, Constant.MEDIACODE_DECODER+"OMXDecoderRank knownCodecList "+ Arrays.toString(mKnownCodecList.keySet().toArray()));
    }
...
//            YLog.debug(OMXConstant.TAG, Constant.MEDIACODE_DECODER+"OMXDecoderRank Codec supported count " + MediaCodecList.getCodecCount());
            for (int i = MediaCodecList.getCodecCount() - 1; i >= 0; i--) {
                MediaCodecInfo info = MediaCodecList.getCodecInfoAt(i);
                if (info.isEncoder()) {
                    continue;
                }
                if (!isSupportMime(info, OMXConstant.CODEC_H264_MIME)) {
                    continue;
                }
                DecoderInfo decoderRankInfo = OMXDecoderRank.instance().getRank(info.getName());
                int rank = decoderRankInfo.rank();

//                YLog.debug(OMXConstant.TAG, Constant.MEDIACODE_DECODER+"OMXDecoderRank codec: %s, rank: %d", info.getName(), rank);
...
//                    YLog.info(OMXConstant.TAG, Constant.MEDIACODE_DECODER+"OMXDecoderRank codec match: %s, rank: %d", info.getName(), rank);
                }
            }
...
//            YLog.error(OMXConstant.TAG,Constant.MEDIACODE_DECODER+"OMXDecoderRank getBestDecoder throwable"+e.getMessage());
            return null;
        }
    }
