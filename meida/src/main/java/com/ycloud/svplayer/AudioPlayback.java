...
class AudioPlayback {
    ...
    public void init(MediaFormat format) {
        ...
        if (mAudioPlaybackRateProcessor != null) {
            ...
            mAudioPlaybackRateProcessor.init(mSampleRate, mChannelCount, false);
        }
        ...
        mLastAudioTrackPlaybackHeadPosition = PTS_NOT_SET;
    ...
    public void flush() {
        if(isInitialized()) {
            ...
            mAudioTrack.flush();
            YYLog.info(TAG,"flush playbackHeadPositionUs: "+getCurrentPresentationTimeUs()+ ",bufferQueue size:" + mBufferQueue.bufferQueue.size());
            mBufferQueue.flush();
            if (mAudioPlaybackRateProcessor != null) {
                mAudioPlaybackRateProcessor.clear();
            }
    ...
    public void write(ByteBuffer audioData, long presentationTimeUs) {
        ...
        if(mFrameChunkSize < sizeInBytes) {
            YYLog.info(TAG, "incoming frame chunk size increased to " + sizeInBytes);
        ...
        synchronized (this) {
            if (mPresentationTimeOffsetUs == PTS_NOT_SET) {
                ...
                if (playbackHeadPositionUs > 0) {
                    ...
                    YYLog.info(TAG, "playback head not reset");
                }
            }
        }

        if (mAudioPlaybackRateProcessor != null) {
            if (mSoundtouchBuffer == null || sizeInBytes > mSoundtouchBuffer.length) {
                mSoundtouchBuffer = new byte[sizeInBytes];
            }
            audioData.get(mSoundtouchBuffer, 0, sizeInBytes);
            mAudioPlaybackRateProcessor.push(mSoundtouchBuffer, sizeInBytes);
            while (true) {
                int len = mAudioPlaybackRateProcessor.pull(mSoundtouchBuffer, 0, sizeInBytes);
                if (len > 0) {
                    mBufferQueue.put(mSoundtouchBuffer, len);
            ...
            YYLog.debug(TAG, "buffer queue size " + mBufferQueue.bufferQueue.size()
    ...
    private void stopAndRelease(boolean killThread) {
        if(killThread && mAudioThread != null) {
            YYLog.warn(TAG, "stopAndRelease interrupt audio thread!!!");
            mAudioThread.interrupt();
            //Audio thread中断后，audio buffer中的音频数据不会被播放出来，清空
            mBufferQueue.bufferQueue.clear();
        }
    ...
    public void stop() {
        if (mAudioTrack != null) {
            if (isInitialized()) {
                while(mBufferQueue.bufferQueue.size() > 0 && !mAudioThread.mPaused){
                    try {
                        YYLog.error(TAG,"stop sleep 100, bufferQueue size:" + mBufferQueue.bufferQueue.size());
                        Thread.sleep(100);
                ...
                mAudioTrack.stop();
                synchronized (this) {
                    YYLog.info(TAG, "playbackHeadPositionUs after stop: " + getPlaybackheadPositionUs());
                    mPresentationTimeOffsetUs = PTS_NOT_SET;
                    mLastPlaybackHeadPositionUs = 0;
                    mLastAudioTrackPlaybackHeadPosition = PTS_NOT_SET;
                }
    ...
    private long getPlaybackheadPositionUs() {
        ...
        if (mLastAudioTrackPlaybackHeadPosition == PTS_NOT_SET) {
            mLastAudioTrackPlaybackHeadPosition = playbackHeadPosition;
            mCustomPlaybackHeadPosition = playbackHeadPosition;
            mLastPlaybackHeadPositionUs = playbackHeadPosition;
        }
        long tmp = playbackHeadPosition;
        playbackHeadPosition = (long) (mCustomPlaybackHeadPosition + (double)(playbackHeadPosition - mLastAudioTrackPlaybackHeadPosition) * mSpeed);
        mLastAudioTrackPlaybackHeadPosition = tmp;
        mCustomPlaybackHeadPosition = playbackHeadPosition;
    ...
    public long getCurrentPresentationTimeUs() {
        ...
        synchronized (this) {
            ...
            long playbackHeadPositionUs = getPlaybackheadPositionUs();
//        YYLog.info(TAG,"playbackHeadPositionUs:" + playbackHeadPositionUs);
            ...
            if (playbackHeadPositionUs < mLastPlaybackHeadPositionUs) {
                ...
                YYLog.info(TAG, "playback head has wrapped " + playbackHeadPositionUs + " >> " + mLastAudioTrackPlaybackHeadPosition + " >> " + mLastPlaybackHeadPositionUs + " >> " + mPresentationTimeOffsetUs);
    ...
    public void setPlaybackSpeed(float speed) {
        ...
        mAudioPlaybackRateProcessor.setRate(speed);
        mSpeed = speed;
    ...
    private void writeToPlaybackBuffer(ByteBuffer audioData, long presentationTimeUs) {
        ...
        //YYLog.info(TAG, "audio write / chunk count " + mPlaybackBufferChunkCount);
    ...
    private class AudioThread extends Thread {
        ...
        void setPaused(boolean paused) {
            YYLog.info(TAG, "setPaused:" + paused);
            mPaused = paused;
            synchronized (this) {
                YYLog.info(TAG, "audio thread notify for paused");
                this.notify();
            }
        }
        ...
        public void run() {
            while(!isInterrupted()) {
                try {
                    synchronized(this) {
                        while(mPaused) {
                            YYLog.info(TAG, "audio thread wait for paused");
                            wait();
                        }
                    }
    ...
    private static class BufferQueue {
        ...
        synchronized void put(byte[] data, int len) {
            if(len > bufferSize) {
            ...
            item.buffer.limit(len);
            item.buffer.mark();
            item.buffer.put(data, 0, len);
            item.buffer.reset();
            item.presentationTimeUs = 0;
