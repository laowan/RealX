...
public class MediaExtractor{
    ...
    private final static String TAG = "MediaExtractor";

    private android.media.MediaExtractor mApiExtractor;
    private boolean bStoreDataInMemory = GlobalConfig.getInstance().getRecordConstant().STORE_DATA_IN_MEMORY == 1 ? true : false;
    private int mMediaType = MediaConst.MEDIA_EXTRACTOR_TYPE_VIDEO;
    private int mUseType = MediaConst.MEDIA_EXTRACTOR_FOR_PLAYER;
    private MemExtractor mMemExtractor = null;
    private int DEFAULT_TRACK_COUNT = 2;

    public MediaExtractor(int mediaType) {
        renewExtractor(mediaType);
    }

    // 用于播放或者导出，最后合成的步骤，边播放边把内存数据送入合成流程，必须存在两个读取索引来区分，以免数据错乱。
    public void setUseType(int useType) {
        if (mMemExtractor != null) {
            mMemExtractor.setUseType(useType);
        }
    }

    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    public int getTrackIndex(String mimeType) {
        for (int i = 0; i < this.getTrackCount(); ++i) {
            MediaFormat format = this.getTrackFormat(i);
            YYLog.info(this, format.toString());
            String mime = format.getString(MediaFormat.KEY_MIME);
    ...
    protected void renewExtractor(int mediaType) {
        mMediaType = mediaType;
        if (mediaType == MediaConst.MEDIA_EXTRACTOR_TYPE_SYSTEM) {
            bStoreDataInMemory = false;
        }
        if(bStoreDataInMemory) {
            if(mMediaType == MediaConst.MEDIA_EXTRACTOR_TYPE_VIDEO) {
                mMemExtractor = new VideoMemExtractor();
            } else {
                mMemExtractor = new AudioMemExtractor();
            }
        } else {
    ...
    public final int getTrackCount() {
        if(!bStoreDataInMemory) {
            ...
            return DEFAULT_TRACK_COUNT - 1;  // only one audio or one video
        }
    }
    ...
    public MediaFormat getTrackFormat(int index) {
        ...
        if(!bStoreDataInMemory) {
             mediaFormat = mApiExtractor.getTrackFormat(index);
        } else {
            mediaFormat = mMemExtractor.getTrackFormat(index);
        }
    ...
    public void seekTo(long timeUs, int mode) {
        if(!bStoreDataInMemory ) {
            ...
            mMemExtractor.seekTo(timeUs, mode);
        }
    }
    ...
    public int readSampleData(ByteBuffer byteBuf, int offset) {
        if(!bStoreDataInMemory ) {
            ...
            return mMemExtractor.readSampleData(byteBuf, offset);
        }
    }
    ...
    public int getSampleTrackIndex() {
        if(!bStoreDataInMemory ) {
            ...
            return mMemExtractor.getSampleTrackIndex();
        }
    }
    ...
    public long getSampleTime() {
        if(!bStoreDataInMemory ) {
            ...
            return mMemExtractor.getSampleTime();
        }
    }
    ...
    public int getSampleFlags() {
        if(!bStoreDataInMemory ) {
            ...
            return mMemExtractor.getSampleFlags();
        }
    }
    ...
    public long getCachedDuration() {
        if(!bStoreDataInMemory ) {
            ...
            return mMemExtractor.getCachedDuration();
        }
    }
